...
{
    public class AddChild: BaseViewModel
    {
        ...
        public AddChild(ViewModel.ChuMo.List lvm)
        {
        ...
        private void SaveModel()
        {
            ...
            {
                int i = 1;
                List<ContentMenuItemsInfo> items = ContentMenuService.GetInstance().MenuInfoList.Items;
                if (items != null && items.Count > 0)
                {
                    i = ContentMenuService.GetInstance().MenuInfoList.Items.Max(u => u.Id) + 1;
                }
                var obj = new ContentMenuItemsInfo()
                {
                    MenuName = _myListViewModel.MenuItemInfoModel.MenuName,
                    MenuItemInfoId= _myListViewModel.MenuItemInfoModel.Id,
                    ZhuTi=_zhuTi,
                    ZuoZhe= _zuoZhe,
                    Id = i,
                    OrderNum = Convert.ToInt32(_orderNum),
                    Stauts = true,
                    Meno = _meno
                };

                ContentMenuService.GetInstance().Add(obj);
                ChildWindow_Closed();
            }
        }
        public void Show()
        {
            ChildWindowManager.Instance.ShowChildWindow(new  View.ChuMo.AddChild() { DataContext = this });
        }
